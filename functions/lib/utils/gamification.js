...
exports.getNextLevelProgress = getNextLevelProgress;
...
async function badgeFirstSteps(_db, _userId, stats) {
    return stats.reports_submitted >= 1;
}
async function badgeProblemSolver(_db, _userId, stats) {
    return stats.reports_resolved >= 5;
}
async function badgeEagleEye(_db, _userId, stats) {
    return stats.reports_resolved >= 10;
}
async function badgeStreaker(db, userId, _stats) {
    ...
    snap.forEach(doc => {
        ...
        if (ts?.toDate)
            weeks.add(getYearWeek(ts.toDate()));
    });
    return weeks.size >= 3;
}
async function badgeZoneHero(db, userId, _stats) {
    const snap = await db.collection('reports')
        .where('citizen_id', '==', userId)
...
async function awardXp(db, userId, amount, reason, options) {
    ...
    const freshStats = freshSnap.data();
    if (!freshStats) {
    ...
    for (const [id, checkFn] of Object.entries(BADGE_CHECKS)) {
        if (freshStats.badges.includes(id))
            continue;
        try {
