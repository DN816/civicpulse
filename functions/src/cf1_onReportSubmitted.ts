...
function calculateSLAHours(severity: string, trustScore: number): number {
  if (severity === 'High') {
    if (trustScore >= 80) return 24;
    if (trustScore >= 50) return 72;
    return 168;
  }
  if (severity === 'Medium') {
    if (trustScore >= 80) return 72;
    return 168;
  }
  return 336;
}
...
function getZoneId(lat: number, lng: number): string {
  if (lat > 30 && lng > 76) return 'A1';
  if (lat > 30 && lng <= 76) return 'A2';
  if (lat <= 30 && lng > 76) return 'B1';
  return 'B2';
}

export const onReportSubmitted = onDocumentCreated(
  {
    ...
    try {
      ...
      if (match) {
        ...
        const updateResult = await db.runTransaction(async (transaction) => {
          ...
          const existingIds: string[] = clusterData.affected_citizen_ids ?? [];
      ...
      const duplicateMatch = await findMatchingCluster(
        classification.category,
        report.lat,
        report.lng,
        deviceTimestamp.toDate()
      ...
      if (duplicateMatch && duplicateMatch.clusterId !== clusterRef.id) {
        ...
        if (existingClusterData) {
          const existingIds: string[] = existingClusterData.affected_citizen_ids ?? [];
          const isNewCitizen = !existingIds.includes(report.citizen_id);
...
async function awardSubmitXp(
  ...
  if (currentSnap.data()?.xp_submit_awarded) return;
  ...
  await awardXp(db, citizenId, 50, 'Report submitted', {
    report_id: reportId,
  ...
  if (isFirstReport) {
    await awardXp(db, citizenId, 100, 'First report bonus', {
      report_id: reportId,
      cluster_id: clusterId ?? undefined,
    });
  }
