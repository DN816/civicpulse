...
export const weeklyHealthReport = onSchedule(
  {
  ...
  async () => {
    ...
    const zoneResolution: Record<string, { resolved: number; total: number }> = {};
    const uniqueCitizenIds = new Set<string>();

    for (const doc of updatedReportsSnap.docs) {
      const data = doc.data();
      const zoneId = (data.zone_id as string) || 'unassigned';
    ...
    const zoneResolutionRates: Record<string, string> = {};
